...
class InsightsNauseaTags extends StatelessWidget {
  const InsightsNauseaTags({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<NauseaProvider>(
        builder: (context, value, child) => value.nauseaTagCounts.isDataLoaded
            ? Column(
                children: [
                  GridOptions(
                    ...
                    elevated: true,
                    options: value.nauseaTagCounts.graphData?.answers?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                  ...
                  GridOptions(
                    title: "Duration",
                    elevated: true,
                    options: value.nauseaTagCounts.graphData?.duration?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                    backgroundColor: AppColors.whiteColor,
                    callback: (index) {},
