...
class InsightsBloatingTags extends StatelessWidget {
  const InsightsBloatingTags({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<BloatingProvider>(
        builder: (context, value, child) => value.bloatingTagCounts.isDataLoaded
            ? Column(
                children: [
                  GridOptions(
                    ...
                    elevated: true,
                    options: value.bloatingTagCounts.graphData?.answers?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                  ...
                  GridOptions(
                    title: "Duration",
                    elevated: true,
                    options: value.bloatingTagCounts.graphData?.duration?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                    backgroundColor: AppColors.whiteColor,
                    callback: (index) {},
