...
class InsightsBrainFogTags extends StatelessWidget {
  const InsightsBrainFogTags({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<BrainFogProvider>(
        builder: (context, value, child) => value.brainFogTagCounts.isDataLoaded
            ? Column(
                children: [
                  GridOptions(
                    ...
                    elevated: true,
                    options: value.brainFogTagCounts.graphData?.answers?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                  ...
                  GridOptions(
                    title: "Duration",
                    elevated: true,
                    options: value.brainFogTagCounts.graphData?.duration?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                    backgroundColor: AppColors.whiteColor,
                    callback: (index) {},
