...
class InsightsEnergyTags extends StatelessWidget {
  const InsightsEnergyTags({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<EnergyProvider>(
        builder: (context, value, child) => value.energyTagCounts.isDataLoaded
            ? GridOptions(
                title: "You have been feeling",
                subtitle: "Your most tracked tags",
                elevated: true,
                options: value.energyTagCounts.energyTags?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                width: 100.w,
                height: 100.h,
