...
class InsightsFatigueTags extends StatelessWidget {
  const InsightsFatigueTags({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<FatigueProvider>(
        builder: (context, value, child) => value.fatigueTagCounts.isDataLoaded
            ? Column(
                children: [
                  GridOptions(
                    ...
                    elevated: true,
                    options: value.fatigueTagCounts.graphData?.answers?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                  ...
                  GridOptions(
                    title: "Duration",
                    elevated: true,
                    options: value.fatigueTagCounts.graphData?.duration?.map((data) => OptionModel(text: data.tag ?? "Unkown", value: data.count)).toList() ?? [],
                    width: 100.w,
                    height: 100.h,
                    backgroundColor: AppColors.whiteColor,
                    callback: (index) {},
