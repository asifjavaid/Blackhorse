...
class DailyTrackerViewHeadacheCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewHeadacheCard({super.key, required this.data});
            ...
            Row(
              children: [
                HelperFunctions.giveBackgroundToIcon(
                    const Icon(
                      AppCustomIcons
                          .headache, // Using brain fog icon as requested
            ...
            data != null
                ? GridView.builder(
                    ...
                    gridDelegate:
                      ...
                      crossAxisSpacing:
                          21, // Horizontal spacing between circles
                      mainAxisSpacing:
                          16.0, // Vertical spacing between rows of circles
                    ),
                    itemCount: (data?.length ?? 0) +
                        1, // Adding an extra circle for the plus icon
                    itemBuilder: (context, index) {
                      return index == data?.length
                          ? const PlusCircle()
                          : NumberCircle(headache: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers headache;

  const NumberCircle({super.key, required this.headache});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Headache]
              ?.contains(headache.id) ??
          false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory(headache.type!,
                                mode: CategoryPanelMode.edit,
                                timeOfDay: headache.timeOfDay)
                            : () => value.handleCategoryDelete(
                                categoryIDs: [headache.id],
                                categoryType: SymptomCategory.Headache),
                        onLongPress: !value.isDeleteMode
                            ? () => value.switchToDeleteMode(true,
                                categoryIDs: [headache.id],
                                categoryType: SymptomCategory.Headache)
                            : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                              side: BorderSide(
                                  width: 2,
                                  color: DailyTrackerUIHelper.getPainScaleColor(
                                      headache.intensityScale ?? 0)),
                              borderRadius: BorderRadius.circular(50),
                            ),
                          ),
                          width: 50,
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(
                          categoryIDs: [headache.id],
                          categoryType: SymptomCategory.Headache),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(headache.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Headache"),
                  child: Container(
                    width: 50,
                    height: 50,
