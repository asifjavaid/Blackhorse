...
class DailyTrackerViewFatigueCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewFatigueCard({super.key, required this.data});
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(fatigue: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers fatigue;

  const NumberCircle({super.key, required this.fatigue});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Fatigue]?.contains(fatigue.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory(fatigue.type!, mode: CategoryPanelMode.edit, timeOfDay: fatigue.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [fatigue.id], categoryType: SymptomCategory.Fatigue),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [fatigue.id], categoryType: SymptomCategory.Fatigue) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                              side: BorderSide(width: 2, color: DailyTrackerUIHelper.getPainScaleColor(fatigue.intensityScale ?? 0)),
                              borderRadius: BorderRadius.circular(50),
                            ),
                          ),
                          width: 50,
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [fatigue.id], categoryType: SymptomCategory.Fatigue),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(fatigue.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Fatigue"),
                  child: Container(
                    width: 50,
                    height: 50,
