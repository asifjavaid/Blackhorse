...
class DailyTrackerViewPainKillersCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewPainKillersCard({super.key, required this.data});
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(painKiller: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers painKiller;

  const NumberCircle({super.key, required this.painKiller});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Pain_Killers]?.contains(painKiller.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory("Pain Killers", mode: CategoryPanelMode.edit, timeOfDay: painKiller.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [painKiller.id], categoryType: SymptomCategory.Pain_Killers),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [painKiller.id], categoryType: SymptomCategory.Pain_Killers) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                              side: BorderSide(width: 2, color: DailyTrackerUIHelper.getMoodScaleColor(painKiller.effectiveScale ?? 0)),
                              borderRadius: BorderRadius.circular(50),
                            ),
                          ),
                          width: 50,
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [painKiller.id], categoryType: SymptomCategory.Pain_Killers),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(painKiller.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Pain Killers"),
                  child: Container(
                    width: 50,
                    height: 50,
