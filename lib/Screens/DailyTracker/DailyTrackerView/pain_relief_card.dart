...
class DailyTrackerViewPainReliefCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewPainReliefCard({super.key, required this.data});
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(painRelief: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers painRelief;

  const NumberCircle({super.key, required this.painRelief});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Pain_Relief]?.contains(painRelief.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory("pain relief", mode: CategoryPanelMode.edit, timeOfDay: painRelief.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [painRelief.id], categoryType: SymptomCategory.Pain_Relief),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [painRelief.id], categoryType: SymptomCategory.Pain_Relief) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                              side: BorderSide(width: 2, color: DailyTrackerUIHelper.getMoodScaleColor(painRelief.enjoyment ?? 0)),
                              borderRadius: BorderRadius.circular(50),
                            ),
                          ),
                          width: 50,
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [painRelief.id], categoryType: SymptomCategory.Pain_Relief),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(painRelief.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("pain relief"),
                  child: Container(
                    width: 50,
                    height: 50,
