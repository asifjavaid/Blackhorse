...
class DailyTrackerViewPregnancyTestCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewPregnancyTestCard({super.key, required this.data});
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(pregnancyTest: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers pregnancyTest;

  const NumberCircle({super.key, required this.pregnancyTest});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Pregnancy_test]?.contains(pregnancyTest.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory(pregnancyTest.type!, mode: CategoryPanelMode.edit, timeOfDay: pregnancyTest.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [pregnancyTest.id], categoryType: SymptomCategory.Pregnancy_test),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [pregnancyTest.id], categoryType: SymptomCategory.Pregnancy_test) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [pregnancyTest.id], categoryType: SymptomCategory.Pregnancy_test),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(pregnancyTest.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Pregnancy test"),
                  child: Container(
                    width: 50,
                    height: 50,
