...
class DailyTrackerViewHormonesCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewHormonesCard({super.key, required this.data});
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(hormones: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers hormones;

  const NumberCircle({super.key, required this.hormones});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Hormones]?.contains(hormones.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory(hormones.type!, mode: CategoryPanelMode.edit, timeOfDay: hormones.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [hormones.id], categoryType: SymptomCategory.Hormones),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [hormones.id], categoryType: SymptomCategory.Hormones) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [hormones.id], categoryType: SymptomCategory.Hormones),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(hormones.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Hormones"),
                  child: Container(
                    width: 50,
                    height: 50,
