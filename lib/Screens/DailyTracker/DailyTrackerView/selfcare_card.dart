...
class DailyTrackerViewSelfcareCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewSelfcareCard({super.key, required this.data});
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(selfcare: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers selfcare;

  const NumberCircle({super.key, required this.selfcare});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Self_Care]?.contains(selfcare.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory("Self-care", mode: CategoryPanelMode.edit, timeOfDay: selfcare.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [selfcare.id], categoryType: SymptomCategory.Self_Care),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [selfcare.id], categoryType: SymptomCategory.Self_Care) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                              side: BorderSide(width: 2, color: DailyTrackerUIHelper.getMoodScaleColor(selfcare.enjoyment ?? 0)),
                              borderRadius: BorderRadius.circular(50),
                            ),
                          ),
                          width: 50,
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [selfcare.id], categoryType: SymptomCategory.Self_Care),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(selfcare.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Self-care"),
                  child: Container(
                    width: 50,
                    height: 50,
