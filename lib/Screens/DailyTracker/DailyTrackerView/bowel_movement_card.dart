...
class DailyTrackerViewBowlMovementCard extends StatelessWidget {
  ...
  const DailyTrackerViewBowlMovementCard({super.key, required this.data});
    ...
    return data.isEmpty
        ...
        : Center(
            child: CircleCard(
              ...
              data: BowelMovementProvider().sortAnswersByTimeOfDay(data),
            ),
          );
  }
}
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(bowlMovement: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final BathroomHabit bowlMovement;

  const NumberCircle({super.key, required this.bowlMovement});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Bowel_movement]?.contains(bowlMovement.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory(bowlMovement.type!, mode: CategoryPanelMode.edit, timeOfDay: bowlMovement.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [bowlMovement.id], categoryType: SymptomCategory.Bowel_movement),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [bowlMovement.id], categoryType: SymptomCategory.Bowel_movement) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                              side: BorderSide(width: 2, color: DailyTrackerUIHelper.getBowelMovementColor(bowlMovement.stoolConsistency ?? 0)),
                              borderRadius: BorderRadius.circular(50),
                            ),
                          ),
                          width: 50,
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [bowlMovement.id], categoryType: SymptomCategory.Bowel_movement),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(bowlMovement.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Bowel Movement"),
                  child: Container(
                    width: 50,
                    height: 50,
