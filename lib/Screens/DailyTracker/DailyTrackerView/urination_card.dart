...
class DailyTrackerViewUrinationCard extends StatelessWidget {
  final List<UrinationUrgencyResponseModel> data;

  const DailyTrackerViewUrinationCard({super.key, required this.data});
    ...
    return data.isEmpty
        ...
        : Center(
            child: CircleCard(
              title: "Urination",
              subtitle: "Urgency",
              data: UrinationProvider().sortAnswersByTimeOfDay(data),
            ),
          );
  }
}
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(urinationUrgency: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final UrinationUrgencyResponseModel urinationUrgency;

  const NumberCircle({super.key, required this.urinationUrgency});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Urination]?.contains(urinationUrgency.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory("urination", mode: CategoryPanelMode.edit, timeOfDay: urinationUrgency.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [urinationUrgency.id], categoryType: SymptomCategory.Urination),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [urinationUrgency.id], categoryType: SymptomCategory.Urination) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [urinationUrgency.id], categoryType: SymptomCategory.Urination),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(urinationUrgency.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("urination"),
                  child: Container(
                    width: 50,
                    height: 50,
