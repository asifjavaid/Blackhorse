...
class DailyTrackerViewAlcoholCard extends StatelessWidget {
  final List<Answers> data;

  const DailyTrackerViewAlcoholCard({super.key, required this.data});
            ...
            data != null
                ? GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return index == data?.length ? const PlusCircle() : NumberCircle(alcohol: data![index]);
                    },
                  )
                : const SizedBox.shrink(),
          ],
...
class NumberCircle extends StatelessWidget {
  final Answers alcohol;

  const NumberCircle({super.key, required this.alcohol});
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DailyTrackerProvider>(builder: (context, value, child) {
      bool isSelected = value.toBeDeletedCategory[SymptomCategory.Alcohol]?.contains(alcohol.id) ?? false;
          ...
          Column(
            children: [
              !isSelected
                  ? Material(
                      ...
                      child: InkWell(
                        ...
                        onTap: !value.isDeleteMode
                            ? () => value.handleSelectedCategory(alcohol.type!, mode: CategoryPanelMode.edit, timeOfDay: alcohol.timeOfDay)
                            : () => value.handleCategoryDelete(categoryIDs: [alcohol.id], categoryType: SymptomCategory.Alcohol),
                        onLongPress: !value.isDeleteMode ? () => value.switchToDeleteMode(true, categoryIDs: [alcohol.id], categoryType: SymptomCategory.Alcohol) : null,
                        onDoubleTap: null,
                        child: Container(
                          decoration: ShapeDecoration(
                            shape: RoundedRectangleBorder(
                  ...
                  : SelectedState(
                      callback: () => value.handleCategoryDelete(categoryIDs: [alcohol.id], categoryType: SymptomCategory.Alcohol),
                    ),
              const SizedBox(height: 4),
              Text(
                HelperFunctions.formatTimeOfDay(alcohol.timeOfDay) ?? "",
  ...
  Widget build(BuildContext context) {
    return Consumer<DailyTrackerProvider>(
        builder: (context, value, child) => Column(
              ...
              children: [
                GestureDetector(
                  onTap: () => value.handleSelectedCategory("Alcohol"),
                  child: Container(
                    width: 50,
                    height: 50,
