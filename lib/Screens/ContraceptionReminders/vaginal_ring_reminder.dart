import 'package:ekvi/Providers/Reminders/vaginal_ring_provider.dart';
import 'package:ekvi/Routes/app_navigation.dart';
import 'package:ekvi/Screens/CycleTracking/cycle_tracking.dart';
import 'package:ekvi/Utils/constants/app_colors.dart';
import 'package:ekvi/Utils/helpers/app_custom_icons.dart';
import 'package:ekvi/Utils/helpers/helper_functions.dart';
import 'package:ekvi/Widgets/Bars/custom_back_navigation_bar.dart';
import 'package:ekvi/Widgets/Buttons/custom_button.dart';
import 'package:ekvi/Widgets/Gradient/gradient_background.dart';
import 'package:ekvi/Widgets/CustomWidgets/notes.dart';
import 'package:flutter/material.dart';
import 'package:intl/intl.dart';
import 'package:provider/provider.dart';
import 'package:sizer/sizer.dart';
import 'package:ekvi/l10n/app_localizations.dart';

class VaginalRingReminder extends StatefulWidget {
  const VaginalRingReminder({super.key});

  @override
  State<VaginalRingReminder> createState() => _VaginalRingReminder();
}

class _VaginalRingReminder extends State<VaginalRingReminder> {
  var provider = Provider.of<VaginalRingProvider>(AppNavigation.currentContext!,
      listen: false);

  @override
  void initState() {
    provider.handleGetVaginalRingReminder();

    super.initState();
  }

  @override
  void dispose() {
    provider.setEnableButton(true, notify: false);
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    TextTheme textTheme = Theme.of(context).textTheme;
    var localizations = AppLocalizations.of(context);
    return Scaffold(
      body: GradientBackground(
        child: SafeArea(
          child: SingleChildScrollView(
            child: Column(
              children: [
                BackNavigation(
                  title: localizations!.vaginalRing,
                  callback: () => AppNavigation.goBack(),
                ),
                Consumer<VaginalRingProvider>(
                  builder: (context, value, child) => Column(
                    children: [
                      TextAndSwitch(
                          text: localizations.vaginalRing,
                          value: value.isVaginalRingReminderSwitchedOn,
                          callback: value.setVaginalRingReminderSwitch),
                      SizedBox(height: 2.h),
                      WhiteContainer(
                          width: 94.w,
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(localizations.ringInsertion,
                                  style: textTheme.headlineSmall),
                              SizedBox(height: 2.h),
                              CustomButton(
                                titleAtStart: true,
                                title: DateFormat("dd/MM/yy")
                                    .format(value.vaginalRingReminderDate),
                                color: AppColors.primaryColor400,
                                fontColor: AppColors.blackColor,
                                tralingIcon: const Icon(
                                  AppCustomIcons.arrow_down,
                                  color: AppColors.actionColor600,
                                  size: 16,
                                ),
                                elevation: 0,
                                onPressed: () => HelperFunctions.showSheet(
                                    context,
                                    child: HelperFunctions.buildDatePicker(
                                        value.vaginalRingReminderDate,
                                        value.setVaginalRingReminderDate),
                                    onClicked: (() => Navigator.pop(context))),
                              ),
                            ],
                          )),
                      SizedBox(height: 2.h),
                      WhiteContainer(
                          width: 94.w,
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(localizations.reminderTime,
                                  style: textTheme.headlineSmall),
                              SizedBox(height: 2.h),
                              Text(localizations.ringChangeReminder,
                                  style: textTheme.labelMedium!
                                      .copyWith(fontWeight: FontWeight.w200)),
                              SizedBox(height: 2.h),
                              CustomButton(
                                titleAtStart: true,
                                title: DateFormat("hh:mm a")
                                    .format(value.vaginalRingReminderTime),
                                color: AppColors.primaryColor400,
                                fontColor: AppColors.blackColor,
                                tralingIcon: const Icon(
                                  AppCustomIcons.arrow_down,
                                  color: AppColors.actionColor600,
                                  size: 16,
                                ),
                                elevation: 0,
                                onPressed: () => HelperFunctions.showSheet(
                                    context,
                                    child: HelperFunctions.buildTimePicker(
                                        value.vaginalRingReminderTime,
                                        value.setVaginalRingReminderTime),
                                    onClicked: (() => Navigator.pop(context))),
                              ),
                            ],
                          )),
                      SizedBox(height: 2.h),
                      Notes(
                        title: localizations.personalizeYourMessage,
                        placeholderText: value.notes,
                        notesText: value.notes,
                        callback: value.setNotes,
                      ),
                      SizedBox(height: 2.h),
                      CustomButton(
                          title: localizations.save,
                          onPressed: value.enableButton
                              ? () => value.handleCreateOrUpdateVaginalRing()
                              : null),
                    ],
                  ),
                )
              ],
            ),
          ),
        ),
      ),
    );
  }
}
