...
class MovementVaultProvider extends ChangeNotifier {
  ...
  String _note = '';
  List<String> _practiceTypeList = ['Select type', "Cardio", "Strength", "Mobility", "Coordination", "Recovery", "Core", "Emotional"];
  ...
  UserMovementsResponseModel? _editingMovement;
  ...
  List<UserMovementsResponseModel> _currentPractices = [];
  List<UserMovementsResponseModel> get currentPractices => _currentPractices.where((practice) => practice.isVisibleInTracker).toList();
  List<UserMovementsResponseModel> get allPractices => _currentPractices.toList();
  List<UserMovementsResponseModel> get previousPractices => _currentPractices.where((practice) => !practice.isVisibleInTracker).toList();
  ...
  void initialiseFields({UserMovementsResponseModel? model, bool notify = false}) {
    _editingMovement = model;
    if (model == null) {
      _practiceName = '';
      _emoji = '⭐️';
  ...
  Future<void> submitPracticeData(BuildContext context) async {
    ...
    try {
      if (_editingMovement != null) {
        UserMovementsResponseModel updatedData = _editingMovement!.copyWith(
          userId: userId!,
          name: _practiceName,
          emoji: _emoji,
          type: practiceType,
        ...
        final result = await MovementService.updatePracticeRecord(_editingMovement!.id!, updatedData);
        result.fold(
          (l) {
        ...
        // Save new record.
        UserMovementsResponseModel data = UserMovementsResponseModel(
          userId: userId!,
          name: _practiceName,
          type: _practiceType,
          emoji: _emoji,
        ...
        Either<dynamic, void> result = await MovementService.savePracticeRecord(data);
        result.fold(
          (l) {
            HelperFunctions.showNotification(AppNavigation.currentContext!, l.toString());
            CustomLoading.hideLoadingIndicator();
  ...
  Future<void> deletePracticeData() async {
    CustomLoading.showLoadingIndicator();
    try {
      final result = await MovementService.deletePracticeRecord(_editingMovement!.id!);
      result.fold(
        (l) {
          HelperFunctions.showNotification(AppNavigation.currentContext!, l.toString());
          CustomLoading.hideLoadingIndicator();
        ...
        (r) {
          _currentPractices.removeWhere((p) => p.id == _editingMovement!.id);
  ...
  Future<void> fetchMovementPractices({bool showLoader = true}) async {
    if (showLoader) CustomLoading.showLoadingIndicator();
    try {
      Either<dynamic, List<UserMovementsResponseModel>> result = await MovementService.getPractices();
      result.fold(
        (l) {
          HelperFunctions.showNotification(AppNavigation.currentContext!, l.toString());
  ...
  Future<void> toggleVisibility(UserMovementsResponseModel practice, bool newVisibility) async {
    ...
    notifyListeners();
    var result = await MovementService.updatePracticeRecord(practice.id!, _currentPractices[index]);
    result.fold(
      (l) {
        HelperFunctions.showNotification(AppNavigation.currentContext!, l.toString());
      },
